...
class human:
    ...
    def __init__(self, speed=HU_PAD_SPEED_SCALE):
        ...
        self.vert_image = pygame.image.load('vert_pad.png')
        self.horiz_image = pygame.image.load('horiz_pad.png')
    ...
    def draw_paddles(self):
            surf.blit(self.vert_image, self.mid_paddle.get_rect())
            surf.blit(self.horiz_image, self.top_paddle.get_rect())
            surf.blit(self.horiz_image, self.bot_paddle.get_rect())
...
class cpu:
    ...
    def __init__(self, speed=CP_PAD_SPEED_SCALE):
        ...
        self.vert_image = pygame.image.load('vert_pad_1.png')
        self.horiz_image = pygame.image.load('horiz_pad_1.png')
...
def play():
    ...
    # Setup music
    pygame.mixer.music.load('another_year.mp3')
    ...
    while not quit_game and not done:
        ...
        # Move CPU
        if b.rect.bottom > c1.mid_paddle.rect.centery and c1.mid_paddle.rect.bottom < c1.bot_paddle.rect.top:
            c1.mid_paddle.rect.top += CP_PAD_SPEED_SCALE
        if b.rect.top < c1.mid_paddle.rect.centery and c1.mid_paddle.rect.top > c1.top_paddle.rect.bottom:
            c1.mid_paddle.rect.top -= CP_PAD_SPEED_SCALE
        if b.rect.left < c1.top_paddle.rect.centerx and c1.top_paddle.rect.left > WINDOWWIDTH/2:
            c1.top_paddle.rect.left -= CP_PAD_SPEED_SCALE
            c1.bot_paddle.rect.left -= CP_PAD_SPEED_SCALE
        if b.rect.right > c1.top_paddle.rect.centerx and h1.top_paddle.rect.right < c1.mid_paddle.rect.right:
            c1.top_paddle.rect.left += CP_PAD_SPEED_SCALE
            c1.bot_paddle.rect.left += CP_PAD_SPEED_SCALE
        ...
        # num_of_dashes = WINDOWHEIGHT/line_length

        # for dash in range(num_of_dashes):
        #     pygame.draw.line(surf, Color("#FFFFFF"), (WINDOWWIDTH/2,dash), (WINDOWWIDTH/2, dash * line_length)

        pygame.draw.line(surf, Color("#FFFFFF"), (WINDOWWIDTH/2,0), (WINDOWWIDTH/2, WINDOWHEIGHT), 2)
